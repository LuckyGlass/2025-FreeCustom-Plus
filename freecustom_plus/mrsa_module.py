"""
This code is modified from https://github.com/aim-uofa/FreeCustom/blob/main/freecustom/mrsa.py
"""
import torch
...
def reshape_mask(ref_mask, mask_weight, H, W):
...
class MultiReferenceSelfAttention():
    def __init__(self,  start_step=0, end_step=50, step_idx=None, layer_idx=None, base_masks=None, ref_masks=None, mask_weights=[1.0,1.0,1.0], style_fidelity=1, viz_cfg=None):
        """
        Args:
            ...
            base_masks   : masks of the referent items in the base image
    ...
    def initialize_base_masks(self, num_ref, H=128, W=128, device='cuda'):
        self.base_masks = torch.zeros((num_ref, H, W), dtype=torch.float32, device=device)
    
    def load_base_masks(self, base_masks):
    ...
    def attn_batch(self, q, k, v, sim, attn, is_cross, place_in_unet, num_heads, **kwargs):
        ...
        if kwargs.get("attn_batch_type") == 'mrsa':
            ...
            sim_or = [sim_own]
            for i, (base_mask, ref_mask, mask_weight) in enumerate(zip(self.base_masks, self.ref_masks, self.mask_weights)):
                ref_mask = reshape_mask(ref_mask, mask_weight, H, W)
                base_mask = reshape_mask(base_mask, 1, H, W)
                concept_mask = torch.outer(base_mask, ref_mask).unsqueeze(0)
                sim_ref = sim_refs[..., H*W*i: H*W*(i+1)]
                sim_ref = sim_ref + concept_mask.masked_fill(concept_mask == 0, torch.finfo(sim.dtype).min)
